...
class Trucking_list extends CI_Controller
{
    ...
    function pdf() {
        ...
        foreach($itemsDetail as $detail) {
            ...
            $pdf->Cell($width[2],5,$detail['packing_batch_number'],'LRTB',0,$position[2]);
            $pdf->Cell($width[3],5,$detail['packing_serial'],'LRTB',0,$position[3]);
            $pdf->Cell($width[4],5,$detail['packing_kg'],'LRTB',0,$position[4]);
